...
fn with_key_type(cfg: &Cfg, rpid: &str, pin: &str, key_type: CredentialSupportedKeyType) -> Result<()> {
...
fn legacy_non_discoverable_credentials(cfg: &Cfg, rpid: &str, pin: &str) -> Result<()> {
    ...
    println!("- Register");
    let challenge = verifier::create_challenge();
    let attestation = ctap_hid_fido2::make_credential(
        cfg,
        rpid,
        &challenge,
        Some(pin),
...
fn legacy_with_key_type(cfg: &Cfg, rpid: &str, pin: &str, key_type: CredentialSupportedKeyType) -> Result<()> {
