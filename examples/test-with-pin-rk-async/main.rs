...
use ctap_hid_fido2::{
    ...
    get_fidokey_devices, verifier, Cfg, FidoKeyHidAsync, FidoKeyHidFactory,
};
...
#[tokio::main(flavor = "current_thread")]async fn main() -> Result<()> {
    ...
    //
    discoverable_credentials(&device, "test-rk.com", pin).await
        .unwrap_or_else(|err| print_error_with_count(&format!("Error => {}\n", err), true));

    with_cred_blob_ex(&device, "test-rk-2.com", pin).await
        .unwrap_or_else(|err| print_error_with_count(&format!("Error => {}\n", err), true));

    //
    // Legacy Pattern Sample
    //
    legacy_discoverable_credentials(&device, "test-rk-legacy.com", pin).await
        .unwrap_or_else(|err| print_error_with_count(&format!("Error => {}\n", err), true));
...
async fn discoverable_credentials(device: &FidoKeyHidAsync, rpid: &str, pin: &str) -> Result<()> {
...
async fn with_cred_blob_ex(device: &FidoKeyHidAsync, rpid: &str, pin: &str) -> Result<()> {
...
async fn legacy_discoverable_credentials(device: &FidoKeyHidAsync, rpid: &str, pin: &str) -> Result<()> {
    ...
    let attestation = device.make_credential_rk(rpid, &challenge, Some(pin), &user_entity).await?;
    ...
    print_step("- Authenticate");
    let challenge = verifier::create_challenge();
    let assertions = device.get_assertions_rk(rpid, &challenge, Some(pin)).await?;
