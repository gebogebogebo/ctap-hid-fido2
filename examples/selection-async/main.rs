...
#[tokio::main(flavor = "current_thread")]async fn main() -> Result<()> {
    ...
    let device = match FidoKeyHidFactory::create_async(&Cfg::init()).await {
        Ok(d) => {
    ...
    let selection_handle = tokio::spawn(async move {
        println!("Selection thread: Attempting authenticatorSelection...");
        match device_for_thread.selection().await {
    ...
    match device_arc.cancel_selection().await {
    ...
    if let Err(e) = selection_handle.await {
