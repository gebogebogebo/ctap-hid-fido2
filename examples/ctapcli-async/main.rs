...
use ctap_hid_fido2::{str_buf, Cfg, FidoKeyHidAsync, FidoKeyHidFactory};
    ...
    if let Some(action) = arg.action {
        match action {
            Action::Info { item, list } => {
                ...
                info::info(&device, &item_val).await?;
            }
            Action::Pin {
                new,
                change,
            ...
            Action::Memo {
                ...
                memo::memo(&device, command).await?;
            }
            Action::Bio {
                list: _,
                info,
            ...
            Action::Cred {
                ...
                cred::cred(&device, command, pin).await?;
            }
            Action::Config {
                toggle_always_uv,
                new_min_pin_length,
                ...
                if toggle_always_uv {
                    cfg::config(&device, cfg::Command::ToggleAlwaysUv, pin).await?;
                    ...
                    cfg::config(&device, cfg::Command::ForceChangePin, pin).await?;
                }
            }
            Action::Blob {
                get,
...
pub async fn up(device: &FidoKeyHidAsync) -> Result<()> {
    if !device.enable_info_param(&InfoParam::VersionsFido21).await? {
        return Err(anyhow!(
            "This authenticator is not supported for this functions."
        ));
    }
    device.selection().await?;

    // If you need to cancel the selection, you can use cid with device.cancel_selection()?;
    device.cancel_selection().await?;
