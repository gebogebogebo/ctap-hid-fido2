...
pub async fn blob(device: &FidoKeyHidAsync, command: Command, pin: Option<String>) -> Result<()> {
    ...
    match command {
        Command::Get => {
            ...
            println!();
            let large_brob_data = device.get_large_blob().await?;
        ...
        Command::Set(write_datas) => {
            ...
            if input == "Yes" {
                device.write_large_blob(Some(&pin), write_datas).await?;
                println!("- done.")
            } else {
                println!("- canceled.")
            }
...
async fn is_supported(device: &FidoKeyHidAsync) -> Result<bool> {
    if device
        .enable_info_option(&InfoOption::LargeBlobs).await?
        .is_some()
    {
        Ok(true)
    } else {
