...
pub async fn pin(device: &FidoKeyHidAsync, command: PinCommand) -> Result<()> {
    match command {
        PinCommand::New => {
            println!("Set new PIN.\n");

            if let Some(client_pin) = device.enable_info_option(&InfoOption::ClientPin).await? {
                if client_pin {
            ...
            println!("new PIN:");
            let pin = common::get_input().await?;
            println!();

            device.set_new_pin(&pin).await?;
        ...
        PinCommand::Change => {
            ...
            if device.enable_info_option(&InfoOption::ClientPin).await?.is_none() {
                return Err(anyhow!("PIN not yet set."));
            };

            println!("current PIN:");
            let current_pin = common::get_input().await?;
            println!();
            println!("new PIN:");
            let new_pin = common::get_input().await?;
            println!();

            device.change_pin(&current_pin, &new_pin).await?;
        ...
        PinCommand::View => {
            ...
            let pin_retries = device.get_pin_retries().await?;
            ...
            let bio_enroll = device.enable_info_option(&InfoOption::BioEnroll).await?;
            if bio_enroll.is_some() && bio_enroll.unwrap() {
                println!();
                println!();
                println!("Get UV retry counter.\n");
                match device.get_uv_retries().await {
                    Ok(v) => {
