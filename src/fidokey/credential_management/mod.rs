...
#[cfg(feature = "tokio")]impl FidoKeyHidAsync {
    /// CredentialManagement - getCredsMetadata (CTAP 2.1-PRE)
    pub async fn credential_management_get_creds_metadata(
        &self,
        pin: Option<&str>,
    ) -> Result<CredentialsCount> {
        let meta = self.credential_management(pin, SubCommand::GetCredsMetadata).await?;
        Ok(CredentialsCount::new(&meta))
    }

    /// CredentialManagement - enumerateRPsBegin & enumerateRPsNext (CTAP 2.1-PRE)
    pub async fn credential_management_enumerate_rps(&self, pin: Option<&str>) -> Result<Vec<Rp>> {
        let mut datas: Vec<Rp> = Vec::new();
        let data = self.credential_management(pin, SubCommand::EnumerateRPsBegin).await?;

        if data.total_rps > 0 {
            datas.push(Rp::new(&data));
            let roop_n = data.total_rps - 1;
            for _ in 0..roop_n {
                let data = self.credential_management(pin, SubCommand::EnumerateRPsGetNextRp).await?;
                datas.push(Rp::new(&data));
            }
        }
        Ok(datas)
    ...
    /// CredentialManagement - enumerateCredentialsBegin & enumerateCredentialsNext (CTAP 2.1-PRE)
    pub async fn credential_management_enumerate_credentials(
        &self,
        pin: Option<&str>,
        rpid_hash: &[u8],
    ) -> Result<Vec<credential_management_params::Credential>> {
    ...
    /// CredentialManagement - deleteCredential (CTAP 2.1-PRE)
    pub async fn credential_management_delete_credential(
        &self,
        pin: Option<&str>,
        pkcd: PublicKeyCredentialDescriptor,
    ) -> Result<()> {
        self.credential_management(pin, SubCommand::DeleteCredential(pkcd)).await?;
        Ok(())
    }

    /// CredentialManagement - updateUserInformation (CTAP 2.1-PRE)
    pub async fn credential_management_update_user_information(
        &self,
        pin: Option<&str>,
        pkcd: PublicKeyCredentialDescriptor,
        pkcue: PublicKeyCredentialUserEntity,
    ) -> Result<()> {
        self.credential_management(pin, SubCommand::UpdateUserInformation(pkcd, pkcue)).await?;
        Ok(())
    }
